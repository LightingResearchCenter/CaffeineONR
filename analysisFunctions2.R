...
Nback_output_mixed_models_caffeine_study_errors <- function(OB, TB, MOB, GNG, post_hoc){
  ...
  hitrate_model <- lme(hit ~ condition2 * light * test_period , random = ~1|Subject_id/condition/test_period,
                    data=hit_rate)
  ...
  fp_rate_model <- lme(fp ~ condition2 * light * test_period , random = ~1|Subject_id/condition/test_period,
                    data=fp_rate)
  sigList_output2 <- pval_postHoc_OutPut("GNG false_positve", fp_rate_model, fp_rate, post_hoc)
  
  
  gng_rt_model<- lme(response_time ~ condition2 * light * test_period , random = ~1|Subject_id/condition/test_period,
                    data=gng_rt)
  sigList_output3 <- pval_postHoc_OutPut("GNG response time", gng_rt_model, gng_rt, post_hoc)

  
  gng_logrt_model<- lme(log_rt ~ condition2 * light * test_period , random = ~1|Subject_id/condition/test_period,
                     data=gng_rt)
  sigList_output4 <- pval_postHoc_OutPut("GNG log(response time)", gng_logrt_model, gng_rt, post_hoc)
  
  
  gng_bot10rt_model<- lme(response_time ~ condition2 * light * test_period , random = ~1|Subject_id/condition/test_period,
                     data=bottem10)
  sigList_output4.1 <- pval_postHoc_OutPut("GNG bottom 10% response time", gng_bot10rt_model, bottem10, post_hoc)
  
  
  gng_top10rt_model<- lme(response_time ~ condition2 * light * test_period , random = ~1|Subject_id/condition/test_period,
                          data=top10)
  sigList_output4.2 <- pval_postHoc_OutPut("GNG top 10% response time", gng_top10rt_model, top10, post_hoc)
  
  
  gng_Logbot10rt_model<- lme(log_rt ~ condition2 * light * test_period , random = ~1|Subject_id/condition/test_period,
                          data=bottem10log)
  sigList_output4.3 <- pval_postHoc_OutPut("GNG bottom 10% log(response time)", gng_Logbot10rt_model, bottem10log, post_hoc)
  
  
  gng_Logtop10rt_model<- lme(log_rt ~ condition2 * light * test_period , random = ~1|Subject_id/condition/test_period,
                          data=top10log)
  ...
  acc_model1 <- lme(accuracy ~ condition2 * light * test_period , random = ~1|Subject_id/condition/test_period,
                    data=accuracy_data00)
  sigList_output5 <- pval_postHoc_OutPut("1-back accuracy", acc_model1, accuracy_data00, post_hoc)
  
  
  acc_model2 <- lme(correct_Match ~ condition2 * light * test_period , random = ~1|Subject_id/condition/test_period,
                    data=accuracy_data01)
  sigList_output6 <- pval_postHoc_OutPut("1-back correct matches", acc_model2, accuracy_data01, post_hoc)
  
  
  acc_model3 <- lme(correct_noMatch ~ condition2 * light * test_period , random = ~1|Subject_id/condition/test_period,
                    data=accuracy_data02)
  sigList_output7 <- pval_postHoc_OutPut("1-back correct no-match", acc_model3, accuracy_data02, post_hoc)
  

  OBrt_model1 <- lme(response_time ~ condition2 * light * test_period , random = ~1|Subject_id/condition/test_period,
                   data=OB2)
  sigList_output8 <- pval_postHoc_OutPut("1-back respone time", OBrt_model1, OB2, post_hoc)
  
  
  OBrt_model2 <- lme(log_rt ~ condition2 * light * test_period , random = ~1|Subject_id/condition/test_period,
                   data=OB2)
  ...
  acc_model4 <- lme(accuracy ~ condition2 * light * test_period , random = ~1|Subject_id/condition/test_period,
                    data=accuracy_data03)
  sigList_output10 <- pval_postHoc_OutPut("2-back accuracy", acc_model4, accuracy_data03, post_hoc)
  

  acc_model5 <- lme(correct_Match ~ condition2 * light * test_period , random = ~1|Subject_id/condition/test_period,
                    data=accuracy_data04)
  sigList_output11 <- pval_postHoc_OutPut("2-back correct matches", acc_model5, accuracy_data04, post_hoc)
  
  
  acc_model6 <- lme(correct_noMatch ~ condition2 * light * test_period , random = ~1|Subject_id/condition/test_period,
                    data=accuracy_data05)
  sigList_output12 <- pval_postHoc_OutPut("2-back correct no-matches", acc_model6, accuracy_data05, post_hoc)
  
 
  TBrt_model1 <- lme(response_time ~ condition2 * light * test_period , random = ~1|Subject_id/condition/test_period,
                   data=TB2)
  sigList_output13 <- pval_postHoc_OutPut("2-back response time", TBrt_model1, TB2, post_hoc)
  
  
  TBrt_model2 <- lme(log_rt ~ condition2 * light * test_period , random = ~1|Subject_id/condition/test_period,
                   data=TB2)
  ...
  acc_model7 <- lme(accuracy ~ condition2 * light * test_period , random = ~1|Subject_id/condition/test_period,
  ...
  acc_model8 <- lme(correct_Match ~ condition2 * light * test_period , random = ~1|Subject_id/condition/test_period,
                    data=accuracy_data07)
  sigList_output16 <- pval_postHoc_OutPut("MOB 1-back correct matches", acc_model8, accuracy_data07, post_hoc)
  
  
  acc_model9 <- lme(correct_noMatch ~ condition2 * light * test_period , random = ~1|Subject_id/condition/test_period,
  ...
  MOBrt_model1 <- lme(response_time ~ condition2 * light * test_period , random = ~1|Subject_id/condition/test_period,
                   data=MOB2)
  sigList_output18 <- pval_postHoc_OutPut("MOB 1-back response time", MOBrt_model1, MOB2, post_hoc)
  
  
  MOBrt_model2 <- lme(log_rt ~ condition2 * light * test_period , random = ~1|Subject_id/condition/test_period,
                   data=MOB2)
